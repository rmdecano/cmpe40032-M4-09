...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        --UPDATE: convert to relative postion to the board
        local mouseCursorX = mouseCursorX - (VIRTUAL_WIDTH - 272)
        local mouseCursorY = mouseCursorY - 16

        --UPDATE: only hightlight the tile if the mouse cursor is within the board
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(
                    function()
                        ...
                        --UPDATE: swap back if there's no match
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            --UPDATE: extends the timer by 1 second per tile in a match
    ...
    --UPDATE: If no matches available, reset the board game
...
function PlayState:render()
    ...
    --UPDATE: draw points gained in the match
    ...
    --UPDATE: Print a message that says there's no possible moves before it reset the board
    if self.printResetMessage then
        ...
        love.graphics.printf('No Possible moves left, Resetting the board', 20, 160, 182, 'center')
    end
end